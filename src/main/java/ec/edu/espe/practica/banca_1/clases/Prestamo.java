...
public class Prestamo {
    ...
    public int monto() {
        ...
        try {// buscar el ultimo mes 
            ...
            resultado = conn.ejecutarSQLSelect("select codigo_movimiento,tipo,fecha"
                    + ",monto,saldo from movimiento where  codigo_cuenta = " + cuenta + " and EXTRACT(MONTH from fecha)= " + Datos[1] + " and EXTRACT(YEAR from fecha)=" + Datos[2] + "");
            while (resultado.next()) {
                Datos[0] = resultado.getString(1);
                System.out.println("codigo  " + Datos[0]);
                Datos[1] = resultado.getString(2);
